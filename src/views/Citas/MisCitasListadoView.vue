            ...
            <div>
              ...
              <div class="mt-2 space-y-1">
                ...
                <p class="text-gray-700">
                  ...
                  Dr. {{ estadisticas.proxima_cita.medico.nombres }} {{ estadisticas.proxima_cita.medico.apellidos }}
                </p>
                <p v-if="estadisticas.proxima_cita.medico.especialidad" class="text-gray-600">
                  {{ estadisticas.proxima_cita.medico.especialidad }}
                </p>
              </div>
            </div>
          </div>
        ...
        >
          <div class="flex items-start justify-between">
            ...
            <div class="flex-1">
              <div class="flex items-start justify-between mb-4">
                <div class="flex items-center space-x-3">
                  <div
                    v-if="cita.medico.foto_url && getFotoUrl(cita.medico.foto_url)"
                    class="w-12 h-12 rounded-full overflow-hidden"
                  >
                    <img
                      :src="getFotoUrl(cita.medico.foto_url)!"
                      :alt="`Dr. ${cita.medico.nombres}`"
                  ...
                  <div>
                    ...
                    <p v-if="cita.medico.especialidad" class="text-sm text-gray-600">
              ...
              <!-- Notas del médico (solo si la cita está completada) -->
              <div v-if="cita.estado === 'completado' && cita.notas" class="p-3 bg-blue-50 rounded-lg mb-4">
              ...
              <div v-if="cita.calificacion" class="flex items-center space-x-2 p-3 bg-yellow-50 rounded-lg">
                ...
                <span class="text-sm font-medium text-gray-900">
                  Tu calificación: {{ cita.calificacion.puntuacion }}/5
                </span>
          ...
          <div class="flex flex-wrap items-center gap-2 mt-4 pt-4 border-t border-gray-200">
            ...
            <Button
              ...
              variant="success"
              size="sm"
              @click="abrirModalConfirmar(cita)"
            >
...
import type { Cita, EstadisticasCitas } from '@/api/citasPacienteService'
...
const estadisticas = ref<EstadisticasCitas | null>(null)
...
const cargarCitas = async () => {
  cargando.value = true
  try {
    const response = await citasPacienteService.obtenerMisCitas(filtros.value)
    if (response.success) {
...
const cargarEstadisticas = async () => {
  try {
    const response = await citasPacienteService.obtenerEstadisticas()
    if (response.success) {
...
const cambiarVista = (vista: string) => {
  router.push({ name: 'paciente-mis-citas-calendario' })
}

// Modales
const abrirModalConfirmar = (cita: Cita) => {
  citaSeleccionada.value = cita
  modales.value.confirmar = true
}

const abrirModalReprogramar = (cita: Cita) => {
  citaSeleccionada.value = cita
...
const abrirModalCalificar = (cita: Cita) => {
  citaSeleccionada.value = cita
  modales.value.calificar = true
}

const abrirModalDetalles = (cita: Cita) => {
  citaSeleccionada.value = cita
